...
@section('title', 'purchase edit form')
@section('content-head', 'Edit purchase')
...
@section('content')
    ...
    @endif
    {!! Form::open(['method'=>'put','route'=>['purchases.update',$purchase->id], 'files'=>true,'class'=>'form-horizontal ']) !!}
    <table  class="table table-responsive mt-2 bordered-0 " >
        <tr>
            <td>
                {{Form::select('supplier_id', $suppliers, $purchase->supplier_id, array('class'=>'form-control supplier_id','id'=>'selector3' ) )}}
            </td>
            <td>
                {{Form::text('purchase_date', $purchase->purchase_date, array('class'=>'form-control item_price','id'=>'datepicker','readonly','placeholder'=>'yyyy-mm-dd' ) )}}

            </td>
            <td>
                {{Form::text('purchase_no',$purchase->purchase_no, array('class'=>'form-control item_price' ) )}}
            </td>
        </tr>
        <tr>
            <td>
    ...
    <table  class="table table-responsive table-striped mt-2 table-bordered" >
        ...
        <tbody id="tablefield">
            <tr>
                <td>
                    {{Form::text('item_id[]', $purchase->item_id, array('class'=>'form-control item_id' ) )}}
                </td>
                <td>
                    {{Form::number('quantity[]', $purchase->quantity, array('class'=>'form-control quantity' ) )}}
                </td>
                <td>
                    {{Form::text('price[]', $purchase->price, array('class'=>'form-control item_price' ) )}}
                </td>
                <td>
                    {{Form::text('discount[]', $purchase->discount, array('class'=>'form-control discount' ) )}}
                </td>
                <td>
                    {{Form::text('amount[]', $purchase->quantity*$purchase->price, array('class'=>'form-control total_price' ) )}}
                </td>
            </tr>
        </tbody>
        <tfoot>
        <tr>
            ...
            <td class="text-center"><p id="sub_total" class="sub_total">{{ $purchase->quantity*$purchase->price }}</p></td>
            <td></td>
        </tr>
        <tr>
            <td colspan="4" class="text-right">Total Discount</td>
            <td class="text-center"><p class="total_discount">{{ $purchase->quantity*$purchase->discount }}</p></td>
            <td></td>
        </tr>
        <tr>
            <td colspan="4" class="text-right"> Total</td>
            <td class="text-center"><p class="total">{{ ($purchase->quantity*$purchase->price)-($purchase->quantity*$purchase->discount) }}</p></td>
            <td></td>
        </tr>
        </tfoot>
    </table>

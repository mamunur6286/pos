...
@section('title', 'Invoice edit form')
@section('content-head', 'Edit Invoice')
...
@section('content')
    ...
    @endif
    {!! Form::open(['method'=>'put','route'=>['invoices.update',$invoice->id], 'files'=>true,'class'=>'form-horizontal ']) !!}
    <table  class="table table-responsive mt-2 bordered-0 " >
        <tr>
            <td>
                {{Form::select('customer_id', $customers, $invoice->customer_id, array('class'=>'form-control customer_id','id'=>'selector3' ) )}}
            </td>
            <td>
                {{Form::text('invoice_date', $invoice->invoice_date, array('class'=>'form-control item_price','id'=>'datepicker','readonly','placeholder'=>'yyyy-mm-dd' ) )}}

            </td>
            <td>
                {{Form::text('invoice_no',$invoice->invoice_no, array('class'=>'form-control item_price' ) )}}
            </td>
        </tr>
        <tr>
            <td>
    ...
    <table  class="table table-responsive table-striped mt-2 table-bordered" >
        ...
        <tbody id="tablefield">
            <tr>
                <td>
                    {{Form::text('item_id[]', $invoice->item_id, array('class'=>'form-control item_id' ) )}}
                </td>
                <td>
                    {{Form::number('quantity[]', $invoice->quantity, array('class'=>'form-control quantity' ) )}}
                </td>
                <td>
                    {{Form::text('price[]', $invoice->price, array('class'=>'form-control item_price' ) )}}
                </td>
                <td>
                    {{Form::text('discount[]', $invoice->discount, array('class'=>'form-control discount' ) )}}
                </td>
                <td>
                    {{Form::text('amount[]', $invoice->quantity*$invoice->price, array('class'=>'form-control total_price' ) )}}
                </td>
            </tr>
        </tbody>
        <tfoot>
        <tr>
            ...
            <td class="text-center"><p id="sub_total" class="sub_total">{{ $invoice->quantity*$invoice->price }}</p></td>
            <td></td>
        </tr>
        <tr>
            <td colspan="4" class="text-right">Total Discount</td>
            <td class="text-center"><p class="total_discount">{{ $invoice->quantity*$invoice->discount }}</p></td>
            <td></td>
        </tr>
        <tr>
            <td colspan="4" class="text-right"> Total</td>
            <td class="text-center"><p class="total">{{ ($invoice->quantity*$invoice->price)-($invoice->quantity*$invoice->discount) }}</p></td>
            <td></td>
        </tr>
        </tfoot>
    </table>
